...
namespace DESKTOP_I_21.View
{
    public partial class PromotionForm : Form
    {
        ...
        private void LoadData()
        {
            ...
            jobBindingSource.DataSource = entities.job.Where(e => e.deleted_at == null && e.id != job.id && e.job_level_id == jl + 1 && e.position.Where(f => f.deleted_at == null).Count() < e.head_count).OrderBy(e => e.job_level_id).ToList();
        }
        ...
        private void dataGridView1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dataGridView1.Rows[e.RowIndex].DataBoundItem is job job)
            {
                ...
                {
                    var applied = job.promotion.Any(f => f.deleted_at == null && f.employee_id == employee.id);
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.Rows[e.RowIndex].DataBoundItem is job job)
            {
                if (e.ColumnIndex == applyButton.Index)
                {
                    var applied = job.promotion.FirstOrDefault(f => f.deleted_at == null && f.employee_id == employee.id);
                    if (applied != null)
                    {
                    ...
                    {
                        entities.promotion.Add(new promotion
                        {
